      ...
      get '/visits/?' do
        with_benchmarking do
          ...
          RedisAnalytics.time_range_formats.each do |range, unit, time_format|
            ...
            @data[range][:visits_new_visits_plot] = @data[range][:new_visits].inject(Hash[@data[range][:visits]]){|a, i| a[i[0]] = [i[1], a[i[0]]];a}.map{|k,v| {'date'=> Time.mktime(*k).strftime(time_format), 'new_visits' => v[0].to_i, 'returning_visits' => v[1].to_i - v[0].to_i}}
            ...
            unique_visits = self.send("#{unit}ly_unique_visits", time_range - 1.send(range)).map{|x,y| [Time.mktime(*x).strftime(time_format), y]}
        ...
        @data[:all_visits] = daily_visits(Time.now - 1.year).map{|x,y| [Time.mktime(*x).to_i * 1000, realistic(y)]}
